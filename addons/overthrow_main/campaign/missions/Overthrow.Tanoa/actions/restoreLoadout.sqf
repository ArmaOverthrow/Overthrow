...
if(count _primary > 0) then {
	...
	if([_ammobox,_pWpn,1] call CBA_fnc_removeWeaponCargoGlobal) then {
		_unit addWeaponGlobal _pWpn;
		if(!isNil "_pAmmo") then {
			if([_ammobox,_pAmmo,1] call CBA_fnc_removeMagazineCargoGlobal) then {
				_unit addMagazineGlobal _pAmmo;
			};
		};
		{
			if([_ammobox,_x,1] call CBA_fnc_removeItemCargoGlobal) then {
...
if(count _secondary > 0) then {
	...
	if([_ammobox,_sWpn,1] call CBA_fnc_removeWeaponCargoGlobal) then {
		_unit addWeaponGlobal _sWpn;
		if(!isNil "_sAmmo") then {
			if([_ammobox,_sAmmo,1] call CBA_fnc_removeMagazineCargoGlobal) then {
...
if(count _tertiary > 0) then {
	...
	if([_ammobox,_tWpn,1] call CBA_fnc_removeWeaponCargoGlobal) then {
		_unit addWeaponGlobal _tWpn;
		if(!isNil "_tAmmo") then {
			if([_ammobox,_tAmmo,1] call CBA_fnc_removeMagazineCargoGlobal) then {
...
if(count _vest > 0) then {
	...
	if([_ammobox,_vestCls,1] call CBA_fnc_removeItemCargoGlobal) then {
		_unit addVest _vestCls;
		{
			...
			_func = CBA_fnc_removeWeaponCargoGlobal;
			[] call {
				if(_cc isKindOf ["ItemCore",configFile >> "CfgWeapons"]) exitWith {					
					_func = CBA_fnc_removeItemCargoGlobal;
				};
				if(_cc isKindOf ["Default",configFile >> "CfgMagazines"]) exitWith {					
					_func = CBA_fnc_removeMagazineCargoGlobal;
				};				
			};	
...
if(count _backpack > 0) then {
	...
	if([_ammobox,_bpCls,1] call CBA_fnc_removeBackpackCargoGlobal) then {
...
if(count _optic > 0) then {
	...
	if([_ammobox,_opticCls,1] call CBA_fnc_removeWeaponCargoGlobal) then {
...
if([_ammobox,_headgear,1] call CBA_fnc_removeItemCargoGlobal) then {
	_unit addHeadgear _headgear;
};

if([_ammobox,_goggles,1] call CBA_fnc_removeItemCargoGlobal) then {
