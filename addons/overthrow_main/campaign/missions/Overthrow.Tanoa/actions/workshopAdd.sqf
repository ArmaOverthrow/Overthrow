...
[_veh,_cls,_price] spawn {
	...
	_veh setVariable ["OT_attached",_cls,true];
	_veh setVariable["AUG_Attached",_wpn,true];
	_veh setVariable["AUG_Local",false,true];
	
	[_veh] call AUG_AddAction;
	
	_veh animate ["hideRearDoor",1]; 
	_veh animate["hideSeatsRear",1];
	
	[[_wpn,"GetOut",{(_this select 2) moveInany (attachedTo(_this select 0)); 	doGetOut (_this select 2); }],"mpAddEventHand",true,true] spawn BIS_fnc_MP;
