OT_objectiveData = [
    [[10098.7,11818.1,0],"Comms Alpha"],
    [[2263.18,6333.35,0],"fuel depot"],
    [[4215.39,11667.7,0],"The Harbor Remnants"],
    [[4132.81,12422.4,0],"The Maze"],
    [[3314.57,12861.9,0],"The Firing Range"],
    [[3809.57,13489.6,0],"The Vehicle Range"],
    [[3821.88,14035.8,0],"The Camp Remnants"],
    [[3123.96,10948.8,0],"Belfort Naval Base"]
];
OT_NATOregion = "island_5";
OT_NATO_HQ = "Tuvanaka Airbase";
OT_NATO_priority = ["Tuvanaka Airbase","Comms Alpha","96FM Georgetown","Comms Whiskey","Lijnhaven","Blue Pearl industrial port","Nani","Belfort","Tuvanaka"];
OT_NATO_Navy_HQ = "Belfort Naval Base";
