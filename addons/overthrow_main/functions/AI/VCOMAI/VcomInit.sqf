...
if (isServer) then
{
	if (isFilePatchingEnabled) then
	{
		...
		{
			private _Settings = compile preprocessFileLineNumbers "\overthrow_main\functions\AI\VCOMAI\Functions\VCOMAI_DefaultSettings.sqf";
			[] call _Settings;
			[Vcm_Settings] remoteExec ["VCM_PublicScript",0,false];
		};
	}
...
if (VCM_FFEARTILLERY) then {nul = [] execVM "\overthrow_main\functions\AI\VCOMAI\RYD_FFE\FFE.sqf";VCM_ARTYENABLE = false;};
