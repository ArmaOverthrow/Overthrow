...

{
	...
	if ! ((str _againcheck) == (str _target)) then {_againF = 1};
	...
	if (((count _phaseF) == 2) && (_x == 1) || ((count _phaseF) == 1)) then {_spotterF = 1};
...
waitUntil 
{
	...
	_available = true;
	//if (isNull _battery1) then {_alive = false};
	if (({! (isNull _x)} count _batlead) < 1 || {({(alive _x)} count _batlead) < 1}) then {_alive = false};
