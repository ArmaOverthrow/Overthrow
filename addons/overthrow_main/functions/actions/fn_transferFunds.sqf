...
OT_inputHandler = {
	...
	if(_val > 0) then {
		...
		format["Transferred $%1 resistance funds to %2",[_val, 1, 0, true] call CBA_fnc_formatNumber,players getvariable [format["name%1",_uid],"player"]] call OT_fnc_notifyMinor;
	};
};
