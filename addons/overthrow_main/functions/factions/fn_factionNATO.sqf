	...
	if(_numplayers > 0) then {
		...
		if(_lastmin != (date select 4)) then {
			...
			if(count _schedule > 0) then {
				_item = [];
				if((_lastsched isEqualTo -1 or _lastsched isEqualTo 0) && _lastmin >= 30) then {
					_lastsched = 30;
					_item = _schedule select 0;
				}else{
					if((_lastsched isEqualTo -1 or _lastsched isEqualTo 30) && _lastmin < 30) then {
						_lastsched = 0;
						_item = _schedule select 0;
					};
				};
		...
		if !(_countered) then {
			{
				...
				if !(_name in _abandoned) then {
					if(_pos call OT_fnc_inSpawnDistance) then {
						...
						_numres = {side _x isEqualTo resistance or captive _x} count (_pos nearObjects 500);
		...
		//Abandon towers
		{
			...
			if !(_name in _abandoned) then {
				if(_pos call OT_fnc_inSpawnDistance) then {
					...
					_numres = {side _x isEqualTo resistance or captive _x} count (_pos nearObjects ["CAManBase",100]);
					if(_nummil < _numres) then {
		...
		_clearedFOBs = [];
		{
			...
			_nummil = {side _x isEqualTo west} count (_pos nearObjects ["CAManBase",300]);
			_numres = {side _x isEqualTo resistance or captive _x} count (_pos nearObjects 50);
		...
		if(_count >= _nextturn && !_countered) then {
			...
			//Abandon towns (under 50 pop) && counter
			_lastcounter = server getVariable ["NATOlastcounter",""];
			{
				...
				if(_pos call OT_fnc_inSpawnDistance) then {
					_nummil = {side _x isEqualTo west} count (_pos nearObjects ["CAManBase",300]);
					_numres = {side _x isEqualTo resistance or captive _x} count (_pos nearObjects 200);
					...
					if(_population < 100) then {
						if(_stability isEqualTo 0 && !(_town in _abandoned)) then {
							//Abandon a town
							_abandoned pushback _town;
							server setVariable [format ["garrison%1",_town],0,true];
							format["NATO has abandoned %1",_town] remoteExec ["OT_fnc_notifyGood",0,false];
			...
			//Spawn missing drones & counter objectives
			{
				...
				if !(_name in _abandoned) then {
					_drone = spawner getVariable [format["drone%1",_name],objNull];
					if((isNull _drone or !alive _drone) && _resources > 10) then {
						_targets = [];
						{
							_town = _x;
						...
						{
							_x params ["_ty","_p"];
							if((_ty isEqualTo "FOB") && (_p distance _pos) < 3000) then {
								_targets pushback _p;
							};
						}foreach(_knownTargets);
			...
			if(_spend > 0) then {
				_targets = [_knownTargets,[],{_x select 2},"DESCEND"] call BIS_fnc_sortBy;
				{
					...
					if !(_done) then {
						if(_ty isEqualTo "WH" or _ty isEqualTo "PS" or _ty isEqualTo "WS") then {
