...
NATO_diff = server getVariable ["OT_difficulty",1];
...
NATO_lastmin = date select 4;
NATO_lastsched = -1;

[{
	params ["_abandoned","_resources"];

	OT_nextNATOTurn = time+(NATO_nextturn * 10);
	publicVariable "OT_nextNATOTurn";

	["factionNATO_loop","_counter%10 isEqualTo 0", "
		...
		if(_numplayers > 0) then {
			...
			if(NATO_lastmin != (date select 4)) then {
				NATO_lastmin = date select 4;
				if(count _schedule > 0) then {
					_item = [];
					if((NATO_lastsched isEqualTo -1 || NATO_lastsched isEqualTo 0) && NATO_lastmin >= 30) then {
						NATO_lastsched = 30;
						_item = _schedule select 0;
					}else{
						if((NATO_lastsched isEqualTo -1 || NATO_lastsched isEqualTo 30) && NATO_lastmin < 30) then {
							NATO_lastsched = 0;
							_item = _schedule select 0;
						};
					};
			...
			if(NATO_count >= NATO_nextturn && !_countered) then {
				...
				NATO_nextturn = OT_NATOwait + random OT_NATOwait;
				OT_nextNATOTurn = time+(NATO_nextturn * 10);
				publicVariable ""OT_nextNATOTurn"";
				...
				_gain = 25;
				_mul = 25;
				if(NATO_diff > 1) then {_gain = 75;_mul = 50};
				if(NATO_diff < 1) then {_gain = 0;_mul = 15};
...

				{
					...
					if !(_name in _abandoned) then {
						...
						if((isNull _drone || !alive _drone) && _resources > 10) then {
							...
							{
								_x params [""_ty"",""_p""];
								if((_ty == ""FOB"") && (_p distance _pos) < 3000) then {
									_targets pushback _p;
								};
							}foreach(_knownTargets);
...
}, [_abandoned,_resources], 100 + (random 300)] call CBA_fnc_waitAndExecute;
