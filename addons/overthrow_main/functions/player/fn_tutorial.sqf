...
OT_menuHandler = {
	...
	_txt = format ["%1<t size='0.5' color='#ffffff'>From here you can perform basic actions such as recruiting civilians or fast travelling to buildings you own, friendly bases && camps that you place. As you can see on the bottom right, this shack is owned by you, so you can therefore fast travel back here when you need to, but not while wanted.<br/><br/>To continue, close this menu (Esc) && open the map (%2 key)</t>",_txt,"ShowMap" call OT_fnc_getAssignedKey];
	sleep 3;
	...
	sleep 3;
	hint "Go && speak to the local gun dealer. Head towards the marked location, you have nothing to worry about as long as you are not carrying/wearing any illegal items.";
	...
	_lines = [
		"Hello? Do I know you?",
		format ["No you don't. My name is %1 && I heard that you might be able to help me",name player],
		...
		_options = [
			[
				"I am sick of NATO pushing us around, what can I do about it?",
				{
					...
					[player,_gundealer,[(_this select 0),"I hear you. I bet it was even them who shot the protester... I tell you what, take this spare pistol I have laying around.","What am I supposed to do with this?","I don't know. But every other guy that's come in here recently that was angry with NATO wanted a gun, && I won't ask questions.","Um.. thanks I guess","No problem, anything you can do to help me stay under their radar is great, I'll pay you $250 if you can take care of them."],_end] spawn OT_fnc_doConversation;
			...
			[
				format ["There's too much crime in %1, && NATO isn't doing anything about it",OT_nation],
				{
					_gundealer = spawner getVariable format["gundealer%1",(getpos player) call OT_fnc_nearestTown];
					private _end = {
			...
			[
				"I want to make some cash, && I don't care about breaking the law",
				{
					_gundealer = spawner getVariable format["gundealer%1",(getpos player) call OT_fnc_nearestTown];
					private _end = {
			...
			[
				"I want to make some cash, legally",
				{
					_gundealer = spawner getVariable format["gundealer%1",(getpos player) call OT_fnc_nearestTown];
					private _end = {
						hint format["Wrecked vehicles can be salvaged with a toolkit, there should be one in your ammo crate at home. Shops on your map are marked with a circle && icon representing what they buy/sell. Towns with lower stability && population will pay higher prices for all items.", "Gear" call OT_fnc_getAssignedKey];
						sleep 10;
						playSound "3DEN_notificationDefault";
